...
if(canvas) {
    ...
    if (!isExhibition && !isArtworks) {
        ...
        for(let i = 0; i < particlesCount; i++) {
            ...
            const R = 15;
            const r_minor = 5 + (Math.random() * 2 - 1);
            
            posArray[i*3] = (R + r_minor * Math.cos(v)) * Math.cos(u);
            posArray[i*3+1] = r_minor * Math.sin(v);
            posArray[i*3+2] = (R + r_minor * Math.cos(v)) * Math.sin(u);
            
            colorsArray[i*3] = 0.3; colorsArray[i*3+1] = 0.4; colorsArray[i*3+2] = 0.5;
        }
    ...
    mainParticlesMaterial = new THREE.PointsMaterial({
        size: 0.35, // 부드러운 가우시안 맵을 씌우면 크기가 작아보이므로 살짝 키움 (0.25 -> 0.35)
    ...
    const dustMaterial = new THREE.PointsMaterial({
        size: 1.5, // 3.5 -> 1.5 (너무 크지 않게 절충)
        map: splatTexture, 
        vertexColors: true,
        transparent: true,
        opacity: 0.12, // 전체적으로 더 희미하게
        blending: THREE.AdditiveBlending,
        depthWrite: false
    });
...
function createMiniScene(containerId, isSignal = false) {
    ...
    if(isSignal) {
        ...
        for(let i=0; i<particlesCount; i++) {
            ...
            const v = Math.random() * Math.PI * 2;
            const R = 2.5;
            const r = 1.0 + (Math.random() * 0.3 - 0.15);
            const x = (R + r * Math.cos(v)) * Math.cos(u);
            const y = (R + r * Math.cos(v)) * Math.sin(u);
            const z = r * Math.sin(v);
            posArray[i*3] = x; posArray[i*3+1] = y; posArray[i*3+2] = z;
            colorArray[i*3] = 0.4; colorArray[i*3+1] = 0.8; colorArray[i*3+2] = 0.9;
        }
    ...
    const material = new THREE.PointsMaterial({
        size: 0.18, // 0.12에서 약간 키움 (가우시안 텍스처로 인해 부드러워짐)
        map: splatTexture, // 미니 캔버스에도 3DGS 스플랫 텍스처 적용
        alphaTest: 0.01,
        vertexColors: true,
        transparent: true,
        opacity: 0.8,
    ...
    // 캔버스 내부에 마우스가 있을 때만 반응하도록 로컬 변수 추가
    let localTargetX = 0;
    let localTargetY = 0;
    let currentRotX = 0;
    let currentRotY = 0;
    ...
    container.addEventListener('mousemove', (e) => {
        ...
        // 컨테이너 내부의 마우스 좌표 (-0.5 ~ 0.5)
        const x = (e.clientX - rect.left) / rect.width - 0.5;
        const y = (e.clientY - rect.top) / rect.height - 0.5;
        
        // 요청에 따라 마우스 회전 반경(민감도)을 대폭 높임 (0.5 -> 2.5)
        localTargetX = y * 2.5; 
        localTargetY = x * 2.5; 
    });
...
window.addEventListener('load', () => {
    // 1st box: The Art of Signals (EE x Art)
    createMiniScene('mini-canvas-1', true); 
    // 2nd box: Capturing Space (Tech)
    createMiniScene('mini-canvas-2', false);  
});
